              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-500">{strategyTags.length} tags</span>
                  </div>

                  {/* Enhanced Tags Container */}
                  <div className="relative">
                    <div className="min-h-[120px] p-4 border-2 border-dashed border-slate-200 rounded-xl bg-gradient-to-br from-slate-50 to-blue-50/30 hover:border-blue-300 transition-all duration-300">
                      <div className="flex flex-wrap gap-3">
                        {strategyTags.map((tag, index) => (
                          ...
                          >
                            <div className="flex items-center bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-full px-4 py-2 shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105">
                              ...
                              <Input
                                ...
                                className="bg-transparent border-0 p-0 h-auto text-sm font-medium text-white placeholder-white/70 focus:ring-0 focus:outline-none min-w-[80px] w-auto"
                                style={{ width: `${Math.max(tag.length * 9 + 20, 80)}px` }}
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") {
                                    e.preventDefault()
                                    const newTags = [...strategyTags]
                              ...
                              <button
                                ...
                                className="ml-2 opacity-0 group-hover:opacity-100 transition-opacity hover:text-red-200 p-1 rounded-full hover:bg-white/20"
                                title="Remove tag"
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                        ...
                        <motion.button
                          ...
                          whileTap={{ scale: 0.95 }}
                          className="flex items-center px-4 py-2 border-2 border-dashed border-blue-300 text-blue-600 rounded-full hover:border-blue-500 hover:bg-blue-50 transition-all duration-300 group"
                          onClick={() => {
                      ...
                      {strategyTags.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-8 text-slate-400">
                          ...
                          <p className="text-xs">Click "Add Tag" to create your first strategy tag</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-slate-600">Quick Add Suggestions</label>
                    <div className="flex flex-wrap gap-2">
                      {["SCALPING", "SWING", "HEDGING", "ARBITRAGE", "MOMENTUM", "REVERSAL"].map((suggestion) => (
                        <button
                          key={suggestion}
                          onClick={() => {
                            if (!strategyTags.includes(suggestion)) {
                              setStrategyTags([...strategyTags, suggestion])
                            }
                          }}
                          disabled={strategyTags.includes(suggestion)}
                          className="px-3 py-1 text-xs border border-slate-200 rounded-full hover:border-blue-300 hover:bg-blue-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-slate-100"
                        >
                          {suggestion}
                        </button>
                      ))}
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <div className="flex items-start">
                      ...
                      <div className="text-xs text-blue-700">
                        <p className="font-medium mb-1">Tag Management Tips:</p>
                        <ul className="space-y-1 text-blue-600">
                          <li>• Click on any tag to edit it directly</li>
                          <li>• Press Enter to add a new tag after the current one</li>
                          <li>• Press Backspace on empty tag to delete it</li>
                          <li>• Use descriptive tags for better strategy organization</li>
                        </ul>
                      </div>
                    </div>
                  </div>
...
const AdvancedActionsPopup = ({
  ...
  // Add state for wait seconds in the AdvancedActionsPopup component:
  const [targetWaitSeconds, setTargetWaitSeconds] = useState(0)
  const [slWaitSeconds, setSlWaitSeconds] = useState(0)
      ...
      <div className="bg-white rounded-lg shadow-xl p-6 w-[450px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {/* Wait Seconds for Target */}
            <div className="mt-2 p-2 bg-green-50 rounded-md border border-green-100">
              <div className="flex items-center justify-between">
                <span className="text-xs text-green-700">Wait Seconds</span>
                <div className="flex items-center gap-2">
                  <Input
                    className="h-6 w-16 text-xs"
                    type="number"
                    value={targetWaitSeconds}
                    onChange={(e) => setTargetWaitSeconds(Number(e.target.value))}
                    placeholder="0"
                  />
                  <span className="text-xs text-green-700">sec</span>
                </div>
              </div>
              <p className="text-xs text-green-600 mt-1">Delay before executing target action</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {/* Wait Seconds for Stoploss */}
            <div className="mt-2 p-2 bg-red-50 rounded-md border border-red-100">
              <div className="flex items-center justify-between">
                <span className="text-xs text-red-700">Wait Seconds</span>
                <div className="flex items-center gap-2">
                  <Input
                    className="h-6 w-16 text-xs"
                    type="number"
                    value={slWaitSeconds}
                    onChange={(e) => setSlWaitSeconds(Number(e.target.value))}
                    placeholder="0"
                  />
                  <span className="text-xs text-red-700">sec</span>
                </div>
              </div>
              <p className="text-xs text-red-600 mt-1">Delay before executing stoploss action</p>
            </div>
          </div>
